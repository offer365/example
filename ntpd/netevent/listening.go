...
func (p *_reactor) ListenUnix(addr string, unix UnixHandler) {
	...
	if erl != nil {
		log.Error("ntpd error: ","Listen err, type: %T; value: %q\n", erl, erl)
	} else {
		p.unix_conn[addr] = c
	}
}
...
func (p *_reactor) ListenTcp(port int, tcp TcpClient) {
	...
	if erl != nil {
		log.Error("ntpd error: ","type: %T; value: %q\n", erl, erl)
	} else {
...
func (p *_reactor) listenUdp(addr *net.UDPAddr, udp UdpClient) {
	...
	log.Info("listening on " + strconv.Itoa(addr.Port))
	c, erl := net.ListenUDP("udp", addr)
	if erl != nil {
		log.Errorf("ntpd error: ","type: %T; value: %q\n", erl, erl)
	} else {
